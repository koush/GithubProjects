	...
	$.ajax({
		...
		success: function(msg) {
			...
			$.each(msg.repositories, function(i, val) {
				...
				var item = "<tr><td width='200'>__NAME__</td><td><a href='__URL__'><img border='0' style='max-width:200px; max-height:200px; visibility:hidden;' src='__IMG__' onload='GithubProjects.showImage(this)'/></a></td><td>__DESCRIPTION__</td></tr>".replace("__NAME__", val.name).replace("__DESCRIPTION__", val.description).replace(/__IMG__/g, img).replace("__URL__", url);
				content += item;
			});
			content += "</table>"
